...
/// (equivalente ao <c>html.dark</c> do protótipo), via <c>window.transcribaInterop.setDarkTheme</c>
...
public sealed class BlazorThemeApplicator : IThemeApplicator
{
    ...
    private static async Task ApplyCoreAsync(IJSRuntime jsRuntime, bool darkTheme)
    {
        try
        {
            await jsRuntime.InvokeVoidAsync("transcribaInterop.setDarkTheme", darkTheme);
        }
